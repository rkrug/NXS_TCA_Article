...
Adapted from openalexVectorComp/inst/scripts/prepare_specter2_merged.py to
support both the `proximity` and `adhoc_query` adapters via --adapter.

One-time setup. Run once per adapter per machine.
...
Environment overrides:
    OVC_SPECTER2_PATH   Output directory (takes precedence over the default cache).
