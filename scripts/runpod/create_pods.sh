#!/usr/bin/env bash
# Create and start N RunPod pods from a config file (see pods.conf.embedding /
# pods.conf.bertopic / pods.conf.example). Uses the RunPod REST API directly
# (https://rest.runpod.io/v1/pods) — no runpodctl install required on the
# calling machine.
#
...
#     populated. Prints a ready-to-paste `ssh_host:`/`ssh_port:` block for
#     input/config.yaml's bertopic.configs.<name> entry.
#
# In both cases the script does not edit config.yaml itself — paste the
# printed block in yourself. Also writes hosts.generated.csv (id,name,host,
# port) for later teardown via stop_pods.sh.
#
# Usage:
#   export RUNPOD_API_KEY=...          # required: RunPod account API key
#   cp scripts/runpod/pods.conf.embedding scripts/runpod/pods.conf   # or pods.conf.bertopic
...
if [[ ! -f "${CONFIG_FILE}" ]]; then
  ...
  echo "       copy scripts/runpod/pods.conf.embedding or pods.conf.bertopic to get started." >&2
  exit 1
fi
...
# is merged on top so a config can inject e.g. PUBLIC_KEY / R2 credentials
# without this script needing to know about them by name.
base_env_json="$(
  jq -n \
...
if [[ "${POD_KIND}" == "http" ]]; then
  ...
  echo "Paste this line into input/config.yaml's active embedding config entry (embeddings.configs.<name>):" >&2
  echo "---" >&2
  echo "${paste_block}"
  echo "---" >&2
else
  ...
  echo "Paste the relevant block(s) into input/config.yaml's bertopic.configs.<name> entry:" >&2
  echo "---" >&2
