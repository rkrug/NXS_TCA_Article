...
# IDLE_MIN minutes.
#
# The GPU script (scripts/run_bertopic_gpu.py) touches the heartbeat at
# start + after every step, so an actively-running job keeps the pod
# alive. SSH sessions can also touch it via a manual `touch /work/.heartbeat`
# at the start of a long interactive session, or via a small SSH command
# wrapper if desired.
#
# Required env:
...
if [ -z "${RUNPOD_API_KEY:-}" ]; then
    echo "[idle-watchdog] RUNPOD_API_KEY not set — runpodctl stop will fail when fired" >&2
fi
